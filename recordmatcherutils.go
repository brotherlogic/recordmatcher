...
type getter interface {
	...
	update(ctx context.Context, i int32, match pbrc.ReleaseMetadata_MatchState) error
}
...
func (s *Server) processRecordList(ctx context.Context, recs []int32) error {
	...
	for _, records := range matches {
		if len(records) == 2 {
			if trackNumbers[records[0].GetRelease().InstanceId] == trackNumbers[records[1].GetRelease().InstanceId] {
				if records[0].GetMetadata().Match != pbrc.ReleaseMetadata_FULL_MATCH {
					...
					err := s.getter.update(ctx, records[0].GetRelease().InstanceId, pbrc.ReleaseMetadata_FULL_MATCH)
					if err != nil {
						return err
					}
				}
		...
		if len(records) == 1 && !records[0].GetMetadata().NeedsStockCheck && time.Now().Sub(time.Unix(records[0].GetMetadata().LastStockCheck, 0)) > time.Hour*24*30*6 && records[0].GetMetadata().Keep != pbrc.ReleaseMetadata_KEEPER && s.requiresStockCheck(ctx, records[0]) {
			...
			err := s.getter.update(ctx, records[0].GetRelease().InstanceId, pbrc.ReleaseMetadata_MATCH_UNKNOWN)
			if err != nil {
				return err
			}
		}
		...
		if len(matches) == 1 {
			...
			return s.getter.update(ctx, records[0].GetRelease().InstanceId, pbrc.ReleaseMetadata_NO_MATCH)
		}
	}
